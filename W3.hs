...
measure Leaf = Leaf
measure t@(Node _ l r) = Node (treeSize t) (measure l) (measure r)
...
foldTree f acc (Node x l r) = f x (foldTree f acc l) (foldTree f acc r)
...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix c0 c1) = map saturate $ zipWith (+) (rgb c0) (rgb c1)
  where saturate x
          | x > 1.0   = 1.0
          | otherwise = x

rgb (Darken d c) = map darken (rgb c)
  where darken x = x - (d * x)
